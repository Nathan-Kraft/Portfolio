...
/**
 * @author mhc31
 * @param <E> IVendable Copied the code from the CandyVendingMachine and then
 * refactored and changed some parameters, so it would work as a general Vending
 * Machine.
 */
public class VendingMachine<E> implements IVendingMachine {
    ...
    HashMap<String, Queue<IVendable>> vendingMachine = new HashMap<String, Queue<IVendable>>();
...
//Currency Formatter specific to locale
     ...
     * @param counts
     */
    public VendingMachine(E[] items, int[] counts) {
        for (int i = 0; i < items.length; i++) {
            String key = letters[i].toUpperCase();
            vendingMachine.putIfAbsent(key, new LinkedList());
            for (int c = counts[i]; c > 0; c--) {
                vendingMachine.get(key).add((IVendable) items[i]);
            }
        }
    }

    public HashMap getVendingMachine() {
        return vendingMachine;
    }

    public void setVendingMachine(HashMap vendingMachine) {
            ...
            if (input.equals("Q")) {    //if the user decides to quit then this executes. 
                ...
                try {   //converting string vlue into a double.
                    ...
                    if (moneyIn >= 100) { //executes if value is higher than 100. Machine can't accept that amount. 
                        System.out.println("Invalid Input, Too much money, vending mchine cannot hold amount of money. You too rich");
                        break;
                    } else if (moneyIn < 0) {
    ...
    public String GetMachineInfo() {
        String y = ("This is a general Vending machine." + "\n" + "You are able to choose any vendable item available from the list below, that you would like to purchase and then have the machine vend the item.");
        return y;
    ...
    public String DisplayContents() {
        String x = ("You have chosen a General machine. Here are your options\n");
